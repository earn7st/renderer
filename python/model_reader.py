...
def read_model(model_name, obj_filepath, scene_context_path, shader_type):
    ...
    for mat_name, mat in scene.materials.items():
        ...
        elif (shader_type == "PBR"):

            mat_info = {                
                "albedo": getattr(mat, 'diffuse', [0.8, 0.8, 0.8]),
                "roughness": getattr(mat, 'shininess', 0.5),
                "metallic":  0.0,
                
                "textures": {
                    "albedo_map": get_tex_name(getattr(mat, 'texture', None)),                    
                    "roughness_map": None,
                    "metallic_map": None,
                    "normal_map": get_tex_name(getattr(mat, 'texture_bump', None)),
                    "ao_map": None
                }
            } 
    ...
    print(f"Materials for {model_name} (with textures) read.")
    ...
    print(f"Read {model_name}: {len(global_vertices)} vertices, {submesh_cnt} submeshes.")

    return mat_library, model_info
